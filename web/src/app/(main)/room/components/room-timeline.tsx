...
export function RoomTimeline({ rooms, date }: RoomTimelineProps) {
  ...
  const selectedRoom = useMemo(
    () => rooms.find((room) => room.itemId === selectedRoomId),
  ...
  const slotMaps = useMemo(
    () =>
      new Map(
        rooms.map((room) => [room.itemId, new Map(room.timeSlots.map((slot) => [slot.time, slot.available]))]),
      ),
    [rooms],
  )
  ...
  function handleSelect(roomId: number, time: string) {
    ...
    const next = [...selectedSlots, time].sort(
      (left, right) => allSlots.indexOf(left) - allSlots.indexOf(right),
    )
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-surface">
              <th className="sticky left-0 z-20 min-w-16 border-b border-r border-border bg-surface px-2 py-2 text-left text-xs font-medium text-foreground-muted">
                시간
              </th>
              {rooms.map((room) => (
                <th
                  key={room.itemId}
                  className="min-w-28 border-b border-r border-border px-2 py-2 text-center text-xs font-medium last:border-r-0"
                >
              ...
              <tr key={time} className="group">
                ...
                {rooms.map((room) => {
                  ...
                  return (
                    <td
                      key={room.itemId}
                      className="border-r border-border p-0.5 last:border-r-0"
                    >
                      {hasSlot ? (
                        <button
                          className={cn(
