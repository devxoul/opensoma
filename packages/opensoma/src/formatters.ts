...
export function parseMentoringDetail(html: string, id = 0): MentoringDetail {
  ...
  const rawTitle =
    labels["모집 명"] || labels["제목"] || cleanText(root.querySelector("h1, h2, .title"));
  ...
  return MentoringDetailSchema.parse({
    ...
    attendees: {
      current: extractNumber(
        labels["신청인원"] ||
          labels["현재인원"] ||
          cleanText(root.querySelector(".total-normal")) ||
          "",
      ),
  ...
  return slots
    .map((slot) => {
      ...
      const time =
        hour && minute
          ? `${hour}:${minute}`
          : normalizeTime(cleanText(slot.querySelector("label") ?? slot));
...
export function parseDashboard(html: string): Dashboard {
  ...
  return DashboardSchema.parse({
    ...
    roomReservations: parseDashboardLinks(
      root,
      (href) => href.includes("/itemRent/") || href.includes("/officeMng/"),
    ),
  });
}
...
export function parseNoticeDetail(html: string, id = 0): NoticeDetail {
  ...
  return NoticeDetailSchema.parse({
    ...
    title:
      cleanText(top?.querySelector(".tit")) ||
      labels["제목"] ||
      cleanText(root.querySelector("h1, h2, .title")),
    author,
    createdAt,
    content: contentNode?.innerHTML.trim() ?? "",
  });
...
export function parseEventList(html: string): EventListItem[] {
  return findTableRows(html, 7).map((cells) =>
    EventListItemSchema.parse({
      id:
        extractLinkParam(cells[2], ["qustnrSn", "bbsId", "nttId"]) ||
        extractNumber(cleanText(cells[0])),
...
export function parsePagination(html: string): Pagination {
  ...
  const pageMatch = items
    .find((item) => item.includes("Page"))
    ?.match(/(\d+)\s*\/\s*(\d+)\s*Page/i);
...
function extractDashEtcValue(container: HTMLElement | null | undefined, label: string): string {
  const match = (container?.querySelectorAll("span") ?? []).find((item) =>
    cleanText(item).startsWith(label),
  );
...
function findListText(card: HTMLElement, label: string): string {
  const item = card
    .querySelectorAll(".txt > li")
    .find((entry) => cleanText(entry).startsWith(label));
...
function extractLocationHref(onclick: string | undefined): string {
  const match =
    onclick?.match(/location\.href\s*=\s*['"]([^'"]+)['"]/) ?? onclick?.match(/['"]([^'"]+)['"]/);
  return match?.[1] ?? "";
}
