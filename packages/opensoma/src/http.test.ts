...
describe("SomaHttp", () => {
  test("get sends query params and stores cookies", async () => {
    const fetchMock = mock(async (input: RequestInfo | URL) => {
      expect(String(input)).toBe(
        `https://www.swmaestro.ai/sw/member/user/forLogin.do?menuNo=${MENU_NO.LOGIN}`,
      );
      return createResponse("<html></html>", [
        "JSESSIONID=session-1; Path=/",
        "XSRF-TOKEN=csrf-1; Path=/",
      ]);
    });
    ...
    const fetchMock = mock(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/forLogin.do")) {
        ...
        return createResponse(
          '<form><input type="hidden" name="csrfToken" value="csrf-login"></form>',
  ...
  test("checkLogin returns user identity when logged in, null otherwise", async () => {
    ...
    const notLoggedInMock = mock(async () =>
      createResponse(
        JSON.stringify({ resultCode: "fail", userVO: { userId: "", userSn: 0 } }),
        [],
        "application/json",
      ),
    );
  ...
  test("extractCsrfToken reads hidden input", async () => {
    const fetchMock = mock(async () =>
      createResponse('<input type="hidden" name="csrfToken" value="csrf-token">'),
    );
    globalThis.fetch = fetchMock as typeof fetch;
