...
async function getAction(id: string, options: GetOptions): Promise<void> {
  try {
    ...
    console.log(
      formatOutput(formatters.parseMentoringDetail(html, Number.parseInt(id, 10)), options.pretty),
    );
  } catch (error) {
    handleError(error);
  }
...
async function deleteAction(id: string, options: GetOptions): Promise<void> {
  try {
    ...
    await http.post(
      ...
      buildDeleteMentoringPayload(Number.parseInt(id, 10)),
    );
    console.log(formatOutput({ ok: true }, options.pretty));
  } catch (error) {
    handleError(error);
...
async function applyAction(id: string, options: GetOptions): Promise<void> {
  try {
    ...
    await http.post(
      ...
      buildApplicationPayload(Number.parseInt(id, 10)),
    );
    console.log(formatOutput({ ok: true }, options.pretty));
  } catch (error) {
    handleError(error);
