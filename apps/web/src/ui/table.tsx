...
export function TableRow({ className, children }: TableElementProps) {
  return <tr className={cn('transition-colors hover:bg-muted/50', className)}>{children}</tr>
}

export function TableHead({ className, children }: TableElementProps) {
  return (
    <th className={cn('bg-muted px-4 py-3 text-xs font-semibold tracking-wide text-foreground uppercase', className)}>
      {children}
    </th>
  )
}
