...
export function ResponsiveTable<T>({ items, columns, keyExtractor }: ResponsiveTableProps<T>) {
  ...
  return (
    <>
      ...
      <div className="grid gap-3 md:hidden">
        {items.map((item) => (
          <Card key={keyExtractor(item)} className="space-y-3 rounded-lg border border-border bg-surface p-4">
            {mobileTitleColumn ? (
